...
#define FOREACH_OMPD_ICV(macro)                                                \
  macro(dyn_var, "dyn-var", ompd_scope_thread, 0) macro(                       \
      run_sched_var, "run-sched-var", ompd_scope_task,                         \
      0) macro(stacksize_var, "stacksize-var", ompd_scope_address_space,       \
               0) macro(cancel_var, "cancel-var", ompd_scope_address_space,    \
                        0) macro(max_task_priority_var,                        \
                                 "max-task-priority-var",                      \
                                 ompd_scope_address_space, 0)                  \
      macro(debug_var, "debug-var", ompd_scope_address_space, 0) macro(        \
          nthreads_var, "nthreads-var", ompd_scope_thread,                     \
          0) macro(display_affinity_var, "display-affinity-var",               \
                   ompd_scope_address_space,                                   \
                   0) macro(affinity_format_var, "affinity-format-var",        \
                            ompd_scope_address_space, 0)                       \
          macro(default_device_var, "default-device-var", ompd_scope_thread,   \
                0) macro(tool_var, "tool-var", ompd_scope_address_space,       \
                         0) macro(tool_libraries_var, "tool-libraries-var",    \
                                  ompd_scope_address_space, 0)                 \
              macro(tool_verbose_init_var, "tool-verbose-init-var",            \
                    ompd_scope_address_space,                                  \
                    0) macro(levels_var, "levels-var", ompd_scope_parallel,    \
                             1) macro(active_levels_var, "active-levels-var",  \
                                      ompd_scope_parallel, 0)                  \
                  macro(thread_limit_var, "thread-limit-var", ompd_scope_task, \
                        0) macro(max_active_levels_var,                        \
                                 "max-active-levels-var", ompd_scope_task, 0)  \
                      macro(bind_var, "bind-var", ompd_scope_task, 0) macro(   \
                          num_procs_var, "num-procs-var",                      \
                          ompd_scope_address_space,                            \
                          0) macro(ompd_num_procs_var, "ompd-num-procs-var",   \
                                   ompd_scope_address_space,                   \
                                   0) macro(thread_num_var, "thread-num-var",  \
                                            ompd_scope_thread,                 \
                                            1) macro(ompd_thread_num_var,      \
                                                     "ompd-thread-num-var",    \
                                                     ompd_scope_thread, 1)     \
                          macro(final_var, "final-task-var", ompd_scope_task,  \
                                0) macro(ompd_final_var, "ompd-final-var",     \
                                         ompd_scope_task,                      \
                                         0) macro(ompd_final_task_var,         \
                                                  "ompd-final-task-var",       \
                                                  ompd_scope_task, 0)          \
                              macro(implicit_var, "implicit-task-var",         \
                                    ompd_scope_task, 0)                        \
                                  macro(ompd_implicit_var,                     \
                                        "ompd-implicit-var", ompd_scope_task,  \
                                        0) macro(ompd_implicit_task_var,       \
                                                 "ompd-implicit-task-var",     \
                                                 ompd_scope_task, 0)           \
                                      macro(team_size_var, "team-size-var",    \
                                            ompd_scope_parallel, 1)            \
                                          macro(ompd_team_size_var,            \
                                                "ompd-team-size-var",          \
                                                ompd_scope_parallel, 1)
...
static ompd_rc_t ompd_enumerate_icvs_cuda(ompd_icv_id_t current,
  ...
  *next_id = next_possible_icv;
  ...
  ret =
      callbacks->alloc_memory(std::strlen(ompd_icv_string_values[*next_id]) + 1,
                              (void **)&new_icv_name);
  if (ret != ompd_rc_ok) {
    return ret;
  }
...
ompd_rc_t ompd_enumerate_icvs(ompd_address_space_handle_t *handle,
  ...
  if (handle->kind == OMPD_DEVICE_KIND_CUDA) {
    return ompd_enumerate_icvs_cuda(current, next_id, next_icv_name, next_scope,
                                    more);
  }
  ...
  *next_id = current + 1;
  ...
  std::strcpy(const_cast<char *>(*next_icv_name),
              ompd_icv_string_values[*next_id]);
...
static ompd_rc_t ompd_get_level_cuda(ompd_parallel_handle_t *parallel_handle,
                                     ompd_word_t *val) {
  if (!parallel_handle->ah)
    return ompd_rc_stale_handle;
  ompd_address_space_context_t *context = parallel_handle->ah->context;
  ...
  uint16_t res;
  ompd_rc_t ret =
      TValue(context, parallel_handle->th)
          .cast("ompd_nvptx_parallel_info_t", 0, OMPD_SEGMENT_CUDA_PTX_GLOBAL)
          .access("level")
...
ompd_rc_t ompd_get_icv_from_scope(void *handle, ompd_scope_t scope,
  ...
  if (device_kind == OMPD_DEVICE_KIND_HOST) {
    ...
    case ompd_icv_team_size_var:
      return ompd_get_num_threads_cuda((ompd_parallel_handle_t *)handle,
                                       icv_value);
    default:
      return ompd_rc_unsupported;
    }
