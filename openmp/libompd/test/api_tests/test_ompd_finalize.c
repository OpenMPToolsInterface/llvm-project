...
// RUN: %gdb-test -x %S/test_ompd_finalize.cmd %t 2>&1 | tee %t.out | FileCheck %s
// RUN: %gdb-test -x %S/test_ompd_finalize.cmd2 %t 2>&1 | tee %t.out2 | FileCheck --check-prefix CMD2 %s
#include <stdio.h>
#include <omp.h>
